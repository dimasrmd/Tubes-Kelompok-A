...
func daftarAdmin(data *[10]admin, i int) {
	...
	fmt.Println("=======================")
	fmt.Println("SYSTEM: DATA BERHASIL DIBUAT!")
}

func cekDataAdmin(Nama string, Password string) bool {
	for _, isi := range dataAdmin {
...
func ubadDataPeminjam(data *[1000]peminjam, indeks int, pilihan string) {
	var ubah peminjam
	cek := true
	for cek {
		switch pilihan {
...
func ubahDataAdmin() {
	var real, ubah admin
	var pilihan, konfirmasi, konfirmasi1 string
	var cek bool
	repeat, repeat1, cek1 := true, true, true
	for repeat {
		fmt.Print("Masukkan nama: ")
		fmt.Scan(&real.Nama)
		fmt.Print("Masukkan password: ")
		fmt.Scan(&real.password)
		cek = cekDataAdmin(real.Nama, real.password)
		if cek {
			for repeat1 {
				fmt.Println("---------- DATA YANG INGIN DIUBAH ---------")
				fmt.Println("1. Nama      2. Password")
				fmt.Print("Pilihan: ")
				fmt.Scan(&pilihan)
				switch pilihan {
				case "1":
					...
					if konfirmasi == "KONFIRMASI" {
						for i, val := range dataAdmin {
							if val.Nama == real.Nama && val.password == real.password {
								dataAdmin[i].Nama = ubah.Nama
							}
						}
						fmt.Println("---------- NAMA BERHASIL DIUBAH -----------")
					} else {
						fmt.Println("------------ NAMA GAGAL DIUBAH ------------")
					}
					repeat, repeat1 = false, false
				case "2":
					fmt.Print("Masukkan password baru: ")
					fmt.Scan(&ubah.password)
					fmt.Print("KONFIRMASI: ")
					fmt.Scan(&konfirmasi)
					if konfirmasi == "KONFIRMASI" {
						for i, val := range dataAdmin {
							if val.Nama == real.Nama && val.password == real.password {
								dataAdmin[i].password = ubah.password
							}
						}
					...
					repeat, repeat1 = false, false
				default:
					fmt.Println("SYSTEM: TIDAK VALID!")
					for cek1 {
						fmt.Print("KELUAR? YES/NO: ")
						fmt.Scan(&konfirmasi1)
						switch konfirmasi1 {
						case "YES":
							repeat, repeat1, cek1 = false, false, false
							fmt.Println("-------------------------------------------")
						case "NO":
							cek1 = false
						default:
			...
			fmt.Println("SYSTEM: DATA INVALID!")
			fmt.Println("-------------------------------------------")
		}
	}
}
...
func main() {
	...
	for cek {
		fmt.Println("=============== MENU ===============")
		fmt.Println("1. Login	3. Ubah data akun")
		fmt.Println("2. Buat akun	4. Keluar")
		fmt.Println("====================================")
		fmt.Print("Pilihan: ")
		fmt.Scan(&pilihan1)
		switch pilihan1 {
		case "1":
			...
			fmt.Println("============ Login ============")
			if iLogin == 0 {
				fmt.Println("SYSTEM: DATA MASIH KOSONG!")
				fmt.Println("===============================")
			} else {
				fmt.Print("Nama: ")
				fmt.Scan(&login.Nama)
				...
				fmt.Println("===============================")
				cekLogin := cekDataAdmin(login.Nama, login.password)
				if cekLogin == true {
					cek = false
					cekMenu = true
					fmt.Println("SYSTEM: LOGIN BERHASIL!")
				} else {
		...
		for cekMenu {
			...
			fmt.Println("====================== MENU ======================")
			fmt.Println("1. MENAMBAHKAN	5. MENGURUTKAN")
			fmt.Println("2. MENGUBAH	6. MENAMPILKAN LAPORAN KESELERUHAN")
			fmt.Println("3. MENGHAPUS	7. KEMBALI")
			...
			case "2":
				...
				if iTambah >= 1 {
					...
					if lanjutUbah {
						fmt.Println("-------------------------------------------")
						fmt.Println("Data yang ingin diubah:")
						fmt.Println("1. Nama			4. Status")
						fmt.Println("2. Jumlah pinjaman	5. Kembali")
						fmt.Println("3. Tenor")
						fmt.Print("Pilihan: ")
						fmt.Scan(&pilihanUbah)
						ubadDataPeminjam(&dataPeminjam, indeksUbah, pilihanUbah)
					}
				} else {
					fmt.Println("SYSTEM: DATA KOSONG!")
				}
			...
			case "5":
				...
				for cekCase5 && iTambah >= 1 {
					cek1Case5 := true
					fmt.Println("1. Jumlah Pinjaman	3. Keluar")
					fmt.Println("2. Tenor")
					fmt.Print("Pilihan: ")
					fmt.Scan(&pilihan_5)
					switch pilihan_5 {
	...
	fmt.Println("|Ryan Akeyla Novianto Widodo/103112400081|")
	fmt.Println("==========================================")
}
